...
public class RenderUtils {
	...
	public static void drawChromaRectangle(int x, int y, int width, int height) {
		...
		while(true) {
			if(i+10 <= width) {
				Gui.drawRect(i, y, i+10, height,RenderUtils.astolfoColorsDraw(i, Minecraft.getMinecraft().displayWidth,10000f));
			} else {
				break;
			}
			i+=10;
		...
		if(width-i != 0) {
			for(int h = i; h < width; h++) {
				Gui.drawRect(h, y, h+1, height,RenderUtils.astolfoColorsDraw(h, Minecraft.getMinecraft().displayWidth,10000f));
			}
		}
	}
	...
	public static void renderChromaString(String text, int x, int y) {
		...
		String[] r = text.split("");
		for(String s : r) {
			Minecraft.getMinecraft().fontRendererObj.drawString(s, xPos, y, RenderUtils.astolfoColorsDraw(xPos, Minecraft.getMinecraft().displayWidth, 10000f));

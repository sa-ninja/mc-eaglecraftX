			...
			for(Entity ent : mc.theWorld.loadedEntityList) {
				if(ent instanceof EntityLivingBase) {
					...
					if(posX <= size /2 && posX >= -size /2 && posZ <= size /2 && posZ >= -size /2)
						if(ent == mc.thePlayer) {
							Gui.drawRect((int)(posX - 1 + x + (size / 2)), (int)(posZ - 1 + y + (size / 2)), (int)(posX + 1 + x + (size / 2)), (int)(posZ + 1 + y + (size / 2)), 0xFF00FF00);
						}
						else
							if(!ent.isInvisible() && ent instanceof EntityPlayer) 
								Gui.drawRect((int)(posX - 0.5 + x + (size / 2)), (int)(posZ - 0.5 + y + (size / 2)), (int)(posX + 0.5 + x + (size / 2)), (int)(posZ + 0.5 + y + (size / 2)), 0xFFFF0000);
			}
